...
{
    void* voxels = ResourceManager::getInstance()->loadVolumeData(volume_path, width, height, slices, bytes_per_pixel);
    m_aTexIds[0] = create3dTex(width, height, slices, bytes_per_pixel, voxels);

    unsigned char* tf = ResourceManager::getInstance()->loadTransferFuncion(tf_path, bytes_per_pixel);
    m_aTexIds[1] = createTransferFuncion(bytes_per_pixel, tf);
...
VolumeData::VolumeData(void* voxels, unsigned char* tff, int width, int height, int slices, size_t bytes_per_pixel)
{
    if(voxels == NULL || tff == NULL) {
        std::cerr << "Error: invalid data. (VolumeData::VolumeData(void* voxels, unsigned char* tff, int width, int height, int slices, size_t bytes_per_pixel))" << std::endl;
        return;
    }

    m_aTexIds[0] = create3dTex(width, height, slices, bytes_per_pixel, voxels);
    m_aTexIds[1] = createTransferFuncion(bytes_per_pixel, tff);
...
void VolumeData::loadVolume(std::string path, int width, int height, int slices, int bytes_per_pixel)
{
    ...
    void* voxels = ResourceManager::getInstance()->loadVolumeData(path, width, height, slices, bytes_per_pixel);
    m_aTexIds[0] = create3dTex(width, height, slices, bytes_per_pixel, voxels);
...
{
    unsigned char* tf = ResourceManager::getInstance()->loadTransferFuncion(path, bytes_per_pixel);
    m_aTexIds[1] = createTransferFuncion(bytes_per_pixel, tf);
